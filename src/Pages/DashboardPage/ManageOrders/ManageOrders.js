...
const ManageOrders = () => {
  ...
  const handleStatusChange = (id, status) => {
    ...
    state.ordersState.forEach((order) => {
      if (order._id === id) {
        order.status = status;
      }
      modifiedOrders.push(order);
    });
    state.ordersState(modifiedOrders);
  ...
  useEffect(() => {
    if (state.deleteStatus === 'Pending') {
      var loading = toast.loading('Deleting...Please Wait!!');
    } else if (state.deleteStatus === 'Success' || state.deleteStatus === 'Rejected') {
      toast.dismiss(loading);
  ...
  return (
    <section id={styles.manage__orders}>
      <h1>Manage Order</h1>
      <>
        {/* {state.deleteStatus === 'Pending' && loading}
        {state.deleteStatus === 'Success' && dismiss} */}
        {/* 
        if(state.deleteStatus === 'Success' || state.deleteStatus === 'Rejected'){toast.dismiss(loading)} */}
        {state.deleteError && toast.error(state.deleteError)}
        {!state.ordersState ? (
          ...
          <>
            <Table bordered size='sm'>
              ...
              <tbody>
                {state.ordersState.map((order, idx) => (
                  <tr key={order._id}>
                    <td>{idx + 1}</td>
                    <td>{order.email}</td>
                    <td>
